...
export async function POST(req: NextRequest) {
  try {
    ...
    const { messages, goalContext, goalId } = await req.json()
    ...
    if (typeof goalId === 'string' && goalId) {
      const owns = await prisma.goal.findFirst({ where: { id: goalId, userId: session.user.id }, select: { id: true } })
    ...
    const stream = await streamChatWithMentor(messages, augmentedContext)
